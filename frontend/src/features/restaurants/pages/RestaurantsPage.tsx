...
const RestaurantsPage = () => {
    const { restaurants, pagination, isLoading, isError, error } = useRestaurants({
        page: 1,
        limit: 10,
    });
        ...
        return (
            <div className="max-w-6xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
                <div className="border-2 border-dashed border-slate-200 rounded-2xl p-16 text-center">
                    ...
                    <h3 className="text-slate-700 font-semibold">No restaurants found</h3>
                    <p className="text-slate-400 text-sm mt-1">No restaurants are available right now.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
            <div className="mb-8">
                <h1 className="text-2xl font-bold text-slate-900">Explore Restaurants</h1>
                <p className="text-slate-500 text-sm mt-1">
                    {pagination?.total ?? restaurants.length} restaurants available
                </p>
            </div>
